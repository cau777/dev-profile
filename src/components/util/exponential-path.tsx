...
class ExponentialFunction {
  ...
  public integral(a: number, b: number) {
    ...
    const l1 = 0.5 * Math.log(Math.sqrt(1 + factor2 * Math.exp(2 * this.factor * b)) + 1)
    const l2 = -0.5 * Math.log(Math.sqrt(1 + factor2 * Math.exp(2 * this.factor * b)) - 1)
    const l3 = -0.5 * Math.log(Math.sqrt(1 + factor2 * Math.exp(2 * this.factor * a)) + 1)
    const l4 = 0.5 * Math.log(Math.sqrt(1 + factor2 * Math.exp(2 * this.factor * a)) - 1)
...
export const projection = (() => {
  ...
  return {graphXStart, graphXEnd, graphWidth, graphYStart, graphYEnd, graphHeight}
})()

export const ExponentialPath: ParentComponent<JSX.PathSVGAttributes<SVGPathElement>> = (props) => {
  const offset = projection.graphYStart * 10
  return (
    <path d={`M 0 150 C 92.658 ${150 - (18.037 - offset)}, 149.240 ${150 - (47.848 - offset)}, 193 0`}
          fill={'transparent'} stroke={'currentcolor'} {...props} />
  )
}