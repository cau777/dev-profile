...
export const RocketSuperSection: Component<Props> = (props) => {
  ...
  const careerSvg = <svg class={'absolute top-0'} width={'100%'} height={'100%'} viewBox={`0 0 300 150`}>
    <CareerSvgContents visible={careerVisible()} lineLength={lineLength()}/>
  </svg>
  const careerSection = <section>
    <CareerSection visible={careerVisible()} setLineLength={setLineLength}>
      {careerSvg}
    </CareerSection>
  </section>

  const mathSvg = <svg width={'100%'} height={'100%'} viewBox={`0 0 300 150`}>
    <Graph/>
  </svg>
  ...
  const parent = (
    <div>
      <div class={'fixed z-10 pointer-events-none'} style={{
  ...
  createEffect(() => {
    ...
    const t = Math.max(0, Math.min(1, relativeScroll / (careerSection as HTMLElement).clientHeight))
