...
const CourseLogo: React.FC<{ org: Org, size: number }> = ({ org, size }) => {
    switch (org) {
        ...
        case "Google Cloud Skills Boost":
            return (
                <Image src={"/google.svg"} w={size} alt={org} title={org} />
            )
    }
}
...
const EducationCard: React.FC<{
    title: React.ReactNode, range: React.ReactNode, descr: React.ReactNode, certifications?: React.ReactNode[]
}> = ({
          title,
          range,
          descr,
    ...
    return (
        <Card withBorder={true} radius="md" w="100%">
            ...
            <Text>{range}</Text>
            <Text>{descr}</Text>
            {certifications?.length && <Group>
                {certifications.map((node, i) => (
                    <Text key={i} c={"dimmed"} size={"sm"}>{node}</Text>
                ))}
            </Group>}
        </Card>
    )
        ...
        <SectionWrapper title={"Education"} size={"md"} Icon={IconSchool}>
            <Stack gap="lg" align="start">
                <EducationCard title={"English as second language (C1 level) - Fisk"}
                               ...
                               descr={<>
                                   Studied for 8 years to achieve fluency in English and a C1 grade on the
                                   Michigan English Test (MET)
                               </>} />
                <EducationCard title={"Python and Java track - Jetbrains Academy"} range={"January/2021 - August/2022"}
                               descr={<>
                                   Project-focused programming course that span basic and advanced computer science
                                   topics, best practices, and Python and Java programming languages. Throughout the
                                   course, I have developed over 20 projects available in <Anchor
                                   href={"https://github.com/cau777?tab=repositories"} target={"_blank"}>my
                                   GitHub</Anchor>.
                               </>}
                               certifications={[
                                   <Anchor
                                       c="dimmed"
                                       target={"_blank"}
                                       href={"https://hyperskill.org/certificates/a6d2152b-a9c1-4bbb-8acd-b99ea73d30ab.pdf"}>
                                       Java Certificate
                                   </Anchor>,
                                   <Anchor
                                       c="dimmed"
                                       target={"_blank"}
                                       href={"https://hyperskill.org/certificates/4d49e64d-93de-47c1-b1fa-41fa1df85af0.pdf"}>
                                       Python Certificate
                                   </Anchor>,
                               ]}
                />
